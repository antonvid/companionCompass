	...
	while ( count < max_count || max_count <= 0 )
	{
		...
		if ( len >= HCI_EVENT_HDR_SIZE )
		{
			...
			if ( meta_event->subevent == EVT_LE_ADVERTISING_REPORT )
			{
				 ...
				 *
				 * Changes:
				 * - Added a filter for BLE beacons specific to my dissertation project
				 */
				...
				while ( reports_count-- )
				{
					...
					uint8_t	*data = info->data; // point to start of advertisement data
					int length = info->length;

					int i = 0;

					// BLE advertising data follows a sequence of length, type and value, for each field
					// with this we can parse the incoming ad data to find the values to the fields we want

					while ( i < length ) // loop through each byte of data
					{
						uint8_t field_len = data[i]; // the first byte determines the length of the field
						if (field_len == 0) break;

						uint8_t type = data[i + 1]; // the second byte determines the type of the field
						
						if ( type == 0xFF ) // if field type is Manufacturer Specifc Data then...
						{
							uint8_t *msd = &data[i + 2]; // point to start of  Manufacturer Specific Data
						 		
							uint16_t company_id = msd[0] | (msd[1] << 8); // company id is transmitted little-endian
							uint8_t device_type = msd[2];

							if ( company_id == 0x2901 && device_type == 0x02 ) // if company id and device type are correct then...
							{
								count++; // iterate the counter as we have found a beacon

								if ( reset_timeout != 0 && timeout > 0 ) // reset/restart the alarm timer
									alarm( timeout );
								
								// major/minors are transmitted big-endian
								uint16_t major = (msd[20] << 8) | msd[21];
								uint16_t minor = (msd[22] << 8) | msd[23];
						...
						i += field_len + 1; // goto the next field
					}

					offset = info->data + info->length + 2; // goto the next advertisement
