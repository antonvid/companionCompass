			...
			{	
				...
				while ( reports_count-- )
				{
					...
					while ( i < length )
					{
						...
						if ( type == 0xFF ) // if manufacturer data
						{
							uint8_t *mfg = &data[i + 2];

							uint16_t company_id = mfg[0] | (mfg[1] << 8);

							if ( company_id == 0x2901 && mfg[2] == 0x02 )
							{
								count++;
								...
								uint16_t major = (mfg[20] << 8) | mfg[21];
								uint16_t minor = (mfg[22] << 8) | mfg[23];
